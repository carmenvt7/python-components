...
class DeviceDataManager(IDataMessageListener):
	...
	def handleIncomingMessage(self, resourceEnum: ResourceNameEnum, msg: str) -> bool:
		...
		pass    
		if not msg:
			logging.warning("Se recibió un mensaje vacío o nulo. Ignorando.")
			return False

		logging.info(f"Mensaje entrante recibido para el recurso {resourceEnum}: {msg}")
		
		try:
			# Llama al método auxiliar para analizar y procesar los datos entrantes
			self._handleIncomingDataAnalysis(msg)
			return True
		except Exception as e:
			logging.error(f"Error al procesar el mensaje entrante: {e}")
			return False
	...
	def _handleIncomingDataAnalysis(self, msg: str):
		...
		"""
		logging.debug("Método _handleIncomingDataAnalysis llamado.")
		
		if not msg:
			logging.warning("Mensaje de datos entrantes vacío o nulo. Ignorando.")
			return
		
		try:
			data = DataUtil().jsonToActuatorData(msg)
			if data:
				logging.info("Mensaje convertido con éxito a ActuatorData.")
				self.actuatorAdapterMgr.sendActuatorCommand(data)
			else:
				logging.warning("El mensaje no pudo convertirse a ActuatorData.")
		except Exception as e:
			logging.error(f"Error al analizar los datos entrantes: {e}")
